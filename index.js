...
fs.readdir('./komutlar/', (err, files) => {
    ...
    log(`Komutlar ${files.length} bu kadar şimdi yukleniyor..`);
    files.forEach(f => {
...
const kiltifat = [
 ...
 client.on('userUpdate', async (oldUser, newUser) => {
    var knaveetiket = ["Suny","suny"]
    let sunucu = client.guilds.cache.find(e => e.id === "813401867920474153")
    let knaverol = sunucu.roles.cache.find(a => a.id === "814486009500532766")
    ...
    if (newUser.discriminator.includes(knaveetiket) && !oldUser.username.includes(knaveetiket)) {
      ...
      client.channels.cache.get(`814545484877660201`).send(knaevlog)
            ...
            client.on('userUpdate', async (oldUser, newUser) => {
                var knave31 = ["Suny","suny"]
                let sunucu = client.guilds.cache.find(e => e.id === "813401867920474153")
                let knave1 = sunucu.roles.cache.find(a => a.id === ["814486009500532766","814482237075619900","814482238490148904","814482237814603826","814482239411716106"])
...
${config.hgemoji} **Hesap** **${hesapzaman}** **Önce Açılmış**`)
      ...
      client.on("message", async knaveqwe => {
        ...
        let Knaveembed = new Discord.MessageEmbed().setAuthor(knaveqwe.member.displayName, knaveqwe.author.avatarURL({dynamic: true})).setColor("010000")
