...
if (!kullanıcı) return message.channel.send(new MessageEmbed().setDescription(`Bir kullanıcıyı sese çekmek istiyorsan o kullanıcıyı belirtmen @Mercy/İD gibi.`).setAuthor(message.member.displayName, message.author.avatarURL({ dynamic: true })).setColor('RANDOM').setTimestamp()).then(x => x.delete({timeout: 5000}));
...
await knavemesaj.react(("814517069692534834"))
await knavemesaj.react(("814517114269859841"))
