...
exports.run = async (client, message, args) => {
 ...
 let member = message.guild.member(message.mentions.members.first() || message.guild.members.cache.get(args[0]));
  if (!member ) return message.channel.send(new Discord.MessageEmbed().setDescription('**Bir @Mercy/İD belirtmelisin.**').setColor("RANDOM"));
  ...
  member.roles.set(member.roles.cache.has(config.booster) ? [config.booster , config.cezalırol] : [config.cezalırol]);
     ...
     .setDescription(`<@${member.id}> adlı kullanıcı __${Knavesebep}__ Sebebiyle <@${message.author.id}> Tarafından cezalıya atıldı.`).setColor("RANDOM")
     jaillog.send(embed1)
}
