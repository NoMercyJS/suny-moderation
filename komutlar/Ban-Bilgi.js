...
.setDescription(`${message.author} Komutu kullanmak için <@&${config.banhammer}> yetkisine sahip olman gerek`)
.setColor('RANDOM')
...
.setTimestamp()).then(x => x.delete({timeout: 5000}));
  ...
  .setDescription(`${message.author} bir @Mercy/İD belirtmelisin.`)
  .setColor('RANDOM')
  .setAuthor(message.member.displayName, message.author.avatarURL({ dynamic: true }))
  .setTimestamp()).then(x => x.delete({timeout: 5000}));
  ...
  try {
    ...
    .then(({ user, reason }) => message.channel.send(new MessageEmbed().setAuthor(user.tag, user.avatarURL())
    .setColor('RANDOM')
    ...
    .setTimestamp()
    .setColor('RANDOM')
    .setDescription('Belirtilen İD\'ye Ait Bir Ban Geçmişi Bulunamadı')) 
                               }
};
